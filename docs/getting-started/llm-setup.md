...
Report generation requires an LLM. Supported: **Ollama** (local), **Gemini**, **Claude**, **OpenAI**, **Groq**. Set `LLM_PROVIDER` and, for cloud providers, `LLM_API_KEY`. Full list: [Configuration – LLM](../configuration.md#llm).

## Ollama (local)

No API key. Runs on your machine.

- **Install:** [ollama.ai](https://ollama.ai) or `brew install ollama` (macOS). Then `ollama serve` (API: http://localhost:11434).
- **Model:** `ollama pull llama3.2` (default). Others: `mistral`, `llama2`. Verify: `curl http://localhost:11434/api/tags`.
- **Config:** Defaults `LLM_PROVIDER=ollama`, `LLM_MODEL=llama3.2`, `LLM_BASE_URL=http://localhost:11434`. App in Docker, Ollama on host: `LLM_BASE_URL=http://host.docker.internal:11434`.
...
- **Report fails / timeout:** Check provider and API key. [Troubleshooting](../reference/troubleshooting.md#reports-and-llm).
